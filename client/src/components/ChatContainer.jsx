...
const ChatContainer = () => {
  ...
  const { authUser, onlineUsers = [] } = useContext(AuthContext); // default empty array
  const scrollEnd = useRef();

  const [input, setInput] = useState(""); // fixed state
  ...
  useEffect(() => {
    if (scrollEnd.current && messages) {
      scrollEnd.current.scrollIntoView({ behavior: "smooth" }); // fix typo "behaviour" -> "behavior"
    }
  }, [messages]);
